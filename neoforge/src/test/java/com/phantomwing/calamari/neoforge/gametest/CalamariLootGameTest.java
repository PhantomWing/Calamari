 ...
 * (cod -> calamari) and the LootEvent additions (guardian drops, cat gift).
 * Registered by {@link CalamariGameTests} — 1.21.5 replaced the game-test
 * annotations with registry-driven test functions, so these are plain static
 * methods used as {@code Consumer<GameTestHelper>}. They run against a tiny empty
 * structure ({@code calamari:empty}).
...
public class CalamariLootGameTest {
    ...
    private static void assertEntityTableYields(GameTestHelper helper, EntityType<?> type, Item expected) {
        ...
        if (found <= 0) {
            helper.fail(Component.literal("Expected " + type + " loot to yield " + expected + " over 400 rolls, got none"));
        } else {
            helper.succeed();
        }
    }
    ...
    private static void assertTableYields(GameTestHelper helper, String tablePath, ContextKeySet paramSet, Item expected) {
        ...
        if (found <= 0) {
            helper.fail(Component.literal("Expected " + tablePath + " loot to yield " + expected + " over 400 rolls, got none"));
        } else {
            helper.succeed();
        }
    }
